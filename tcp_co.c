#include <signal.h> // Tratamento de interrupções
#include <stdio.h>
#include <arpa/inet.h> // Conversão de endereços de rede
#include <bpf/libbpf.h>
#include <bpf/bpf.h>
#include <time.h>
#include "tcp_co.skel.h" // Skeleton (ponte entre código BPF e userspace)

// Estruturas do mapa - mesmo padrão declarado no kernel:
...
// Cria o csv para guardar os dados
FILE *csv_file = NULL; 

// Callback - executado quando o kernel envia um evento pelo Ring Buffer
static int handle_event(void *ctx, void *data, size_t data_sz) {
    const struct tcp_event *e = data;
    char src_ip[INET_ADDRSTRLEN], dst_ip[INET_ADDRSTRLEN];
    time_t now = time(NULL);

    // Converte os IPs para string
...
// Tratar a interrupção do programa - sair do loop de polling:
...
int main(int argc, char **argv) {
    ...
    // Abre e carrega o programa eBPF no kernel
    skel = tcp_co_bpf__open_and_load();
    if (!skel) return 1;

    // Anexa o programa na função tcp_rcv_established
    tcp_co_bpf__attach(skel);
    
    // Configuração do Ring Buffer para o mapa declarado no programa epbf
    // Dispara a função handle_event a cada evento recebido
    rb = ring_buffer__new(bpf_map__fd(skel->maps.rb), handle_event, NULL, NULL);

    printf("Gravando dados em %s\n", filename);

    // Intercepta os sinais de parada:

    signal(SIGINT, sig_handler);
    signal(SIGTERM, sig_handler);

    // Polling - fica checando o Ring Buffer a cada 100ms até a interrupção
    while (!exiting) {
        if (ring_buffer__poll(rb, 100) < 0) break;
    }
    
    // Mapa Hash que guarda o último estado conhecido das conexões:
    ...
    // Remove o programa do kernel de forma segura
